      ...
      case "radio-group":
        return (
          <View
            ...
            style={{ marginTop: 8, alignItems :'center', width: '100%', backgroundColor: '#fff', flexDirection: 'row', justifyContent: 'space-between', borderRadius : 5, borderWidth: 1, padding: 5, borderColor: Colors.lightGrey }}
          >
            <Text
              // style={[styles.labelName, styles.width50]}
              style={{ color: Colors.labelColor, fontSize: Colors.lableSize, fontWeight: '500', margin: 10 }}
            >
              {item.label}:
            </Text>
            <RadioForm
        ...
        return (
          <>
            <InputBox
              ...
              rightElement={getOpenStatus(item.id) ? "chevron-up" : "chevron-down"}
              DropDown={() => toggleInputDropdownMenu(item.id, !getOpenStatus(item.id), item.action)}
            ...
            {getOpenStatus(item.id) ? (
              <View style={{ flex: 1, backgroundColor: "#fff" }}>
                <Category
                  ...
                  onCatPress={(value) => setInputDropdownData(value, item.values_for_dropdown, item.id, item.multiple)}
                  heading={"Choose " + item.label}
                  isMulti={item.multiple}
                  onClose={() => toggleInputDropdownMenu(item.id, !getOpenStatus(item.id), item.action)}
                />
              </View>
            ) : null}
    ...
    <>
      ...
      <FlatList
        ...
        contentContainerStyle={
          formData?.length === 0 ? styles.container : null
        }
      />
    </>
  );
