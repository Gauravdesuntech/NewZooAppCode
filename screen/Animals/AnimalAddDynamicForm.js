...
export default class AnimalAddDynamicForm extends Component {
    ...
    render() {
        // console.log({ accTypeData: this.state.accTypeData });
        return (
            <SafeAreaView style={styles.container}>
                <Loader visible={this.state.isLoading} />
                <CustomForm header={true} title={"Add Animals"} marginBottom={60} onPress={this.onSubmit}>
                    <InputBox
                        inputLabel={"Accession Type"}
                        placeholder={"Choose Accession Type"}
                        editable={false}
                        value={this.state.accType}
                        defaultValue={this.state.accType != null ? this.state.accType : null}
                        rightElement={this.state.isaccTypeMenuOpen ? "chevron-up" : "chevron-down"}
                        DropDown={() => this.setState({ isaccTypeMenuOpen: !this.state.isaccTypeMenuOpen })}
                        errors={this.state.errorMessage.accType}
                        isError={this.state.isError.accType}
                    />
                    {this.state.formData == null ? <Text style={{ color: 'tomato', textAlign: 'center' }}>No Form found for this Accession Type!!</Text> :
                        <DynamicForm
                ...
                {this.state.isaccTypeMenuOpen ? (
                    <View style={{ flex: 1, backgroundColor: "#fff" }}>
                        <Category
                            ...
                            isMulti={false}
                            onClose={() => this.setState({ isaccTypeMenuOpen: !this.state.isaccTypeMenuOpen })}
                        />
                    </View>
                ) : null}
            </SafeAreaView>
