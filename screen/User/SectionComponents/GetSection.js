...
import { GetSectionData, deleteSectionData } from "../../../services/AllSectionApis";
...
const GetSection = () => {
    ...
    const InnerComponent = ({ item }) => {
        const { section_id, section_name, section_image, user_id, full_access, active, created_at, modified_at, } = item.item;
        return (
            <View>
    ...
    const deleteSection = (item) => {
        ...
        deleteSectionData(obj).then((res) => {
            if (!res.success) {
                ...
                isLoading || refreshing ? <Loader loaderSize={'lg'} /> :
                    ...
                    <View style={styles.container}>
                        ...
                        <View style={styles.listSection}>
                            <FlatList
                                data={sectionData}
                                renderItem={(item) => <ListComponent item={item} onPressDelete={(item) => deleteSection(item)} onPressEdit={() => EditSectionData(item)} ><InnerComponent item={item} /></ListComponent>}
