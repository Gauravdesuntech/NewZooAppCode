...
const ListAllHatchedStatus = () => {
  ...
  const InnerList = ({ item }) => {
    ...
    const { id, active, comment, created_at, created_by, hatched_key, hatched_type, string_id, updated_at, updated_by } = item.item;
    return (
      <View style={{ flex: 1, }}>
              ...
              <FlatList
                ...
                renderItem={item =>
                  <ListComponent item={item}
                    onPress={() => navigation.navigate('GetHatchedStatus', { itemId: item.item.id })}
                    onPressDelete={() => deleteExperienceData(item)} onPressEdit={() => editExperienceData(item)}
                  >
                    <InnerList item={item} />
                  </ListComponent>}
