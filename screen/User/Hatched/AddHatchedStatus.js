...
const AddHatchedStatus = ({ navigation }) => {
  ...
  const onSubmit = () => {
    ...
    if (hatchedTypeError === false && hatchedCommentError === false && hatchedCommentError === false) {
      setLoding(true)
  ...
  const getHatchedTypeData = (item) => {
    ...
    // console.log("Data is Movable*********", hatchedType);
  }

  return (
    <>
      {loading ? <Loader /> :
        <CustomForm header={true} title={"Add Hatched Status"} onPress={onSubmit}>
          <View>
            <NewDropdown
              title="Hatched Type"
