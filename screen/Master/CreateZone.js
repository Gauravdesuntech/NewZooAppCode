...
export default function CreateZone() {
    ...
    const getZoneData = () => {
        ...
        if (zoneName.trim().length === 0) {
        ...
        else if (zoneDescription.trim().length === 0 || !nameRegex.test(zoneDescription)) {
            setIsError({ zoneDescription: true })
            setErrorMessage({ zoneDescription: "Enter Description" })
            return false;
        } else if (longitude.trim().length === 0) {
            ...
            console.log(obj, ">>>>>>>>>>>")
            setLoding(true)
            CreateZoneService(obj).then((res) => {
                console.log("hello this is response by ashu--->>>>", res);
            })
                .catch((err) => {
                    console.log(err);
                })
                .finally(() => {
                    ...
                    alert("Created Zone Is Added Successfully")
                    console.log("ok ashu =>>>>>>=====");
                })
            setZoneName("")
            setZoneDescription("")
